...
points(nr_predictors, LM_regsubsets_summary$adjr2[nr_predictors_adjr2], col = "red", cex = 2, pch = 20)
```
...
points(nr_predictors, LM_regsubsets_summary$bic[nr_predictors_bic], col = "red", cex = 2, pch = 20)
```
...
points(nr_predictors, LM_regsubsets_summary$cp[nr_predictors_cp], col = "red", cex = 2, pch = 20)
```


That is, the model suggests `r nr_predictors_cp` predictors to be included.

Ok, so let's stick to `r nr_predictors_adjr2` predictors. These are:

```{r}
coef(LM_regsubsets, nr_predictors)[-1]
...
LM_regsubsets_predict <- predict(LM_regsubsets, newdata = test_sample, id = nr_predictors)
# str(dummy_model2)
...
Finally, let's combine model results to derive the importance of variables after having fitted all these models.
...
Let's pick the two most successful models, and let's only consider the variables from those models.
...
So the best two models were `r best_regr_models`.

Now let's look at the variable importance measures from these two variables.
...
varimp_all %>% 
  mutate(model = tolower(model)) %>% 
  filter(str_detect(model, tolower(regr_results$model[2]))) %>% 
  top_n(-10) -> varimp_best_2
...
What predictors appear in both "hit lists"?

```{r varimp_intersect}

base::intersect(varimp_best_1$name, varimp_best_2$name) -> varimp_intersect

varimp_intersect
```

These are (this is) the variable(s) that where among the 10 most important predictors in the two (best performing) models. Let's take them as our "best guess" as the most important ones.
