...
univariate_lm_selection %>% 
  filter(pvalue < .05) -> univariate_lm_selection_signif
...
tibble(
  name = "ADHD1",
  value = summary(lm1)$adj.r.squared,
...
if ("ADHD1" %in% best_preds) best_preds[best_preds == "ADHD1"] <- "ADHD"
```
...
train_sample %>% 
  ...
  do(tidy(lm(train_sample$CYBOCS_3m ~ ., data = .))) -> lm2_bestsub

lm2_bestsub %>% 
  filter(term != "(Intercept)") %>% 
  dplyr::select(term, statistic) %>% 
  dplyr::rename(name = term, value = statistic) %>% 
  mutate(rank = min_rank(desc(value))) -> regr_varimp$lm2_bestsub
...
regr_results %>% 
  ...
  dplyr::rename(`name of model` = name_models, `Rsquared` = R2_models, RMSE = RMSE_models) %>% 
  mutate_if(is.numeric, round, digits = 2) %>% 
  kable
...
do.call("rbind", regr_varimp) %>% 
  ...
  filter(name != "(Intercept)") -> varimp_all
...
But as only Boosting and RF were successful, let's only consider the variables from those models.

```{r varimp_all_boost_rf}

varimp_all %>% 
  filter(str_detect(model, "gbm")) %>% 
  top_n(-10) -> varimp_gbm
...
base::intersect(varimp_gbm$name, varimp_rf$name) -> varimp_intersect

varimp_intersect
```

These are the variables that where among the 10 most important predictors in both (best performing) models. Let's take them as our "best guess" as the most important ones.