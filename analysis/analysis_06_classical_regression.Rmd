...
lm(CYBOCS_3m ~ ., data = data2) %>% summary %>% tidy %>% kable
```
...
data %>% 
  ...
  mutate(predictor = str_sub(predictor, start = 1, end = str_length(predictor)-3))  -> mod_results$classical_01$univariate_lm_selection
...
kable(mod_results$classical_02$univariate_lm_selection_details)
```

Here is the model performance ($R^2$ etc.).
...
kable(mod_results$classical_02$univariate_lm_selection_glance)
```
