...
```{r glmfit3}
library(leaps)
glmfit3 <- regsubsets(responder_3m_f ~ . , data = data_mod2,
                      nvmax = 10, 
                      method = "forward")
...
names(mod_results$glmfit4)
glm4_pred <- predict(glmfit4, test2)
...
Accuracy as measured by Cohen's Kappa in **train** set is `r mod_results$glmfit4$fit$results$Accuracy`.

However, the accuracy in **test** set is `r mod_results$glmfit4$confusion_matrix`.
