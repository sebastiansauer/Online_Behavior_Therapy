...
### Cronbachs Alpha and interitem correlation

Some item were **negatively** correlated with the scale (`ChOCI_R_C_26_PRE-`). They should be checked.
...
### Intercorrelations of psychometric scales

Let's have a look at the sum scores of the psychometric scales to see whether/ to what degree they correlate (assuming metric level).***

```{r}
search_string <- "ChOCI"
data %>% 
  select(matches(search_string), responder_3m) %>% 
  na.omit -> data_scales
  ggpairs(data_scales, mapping = aes(color = responder_3m))
